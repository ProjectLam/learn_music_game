...
class AsyncRequest:
	...
	func parse_result():
		...
		if json_error != OK:
			logger.debug("Unable to parse request %d response. JSON error: %s, response code: %d" % [
				id, json.get_error_message(), response_code
			])
...
func send_async(p_method : String, p_uri : String, p_headers : Dictionary, p_body : PackedByteArray):
	...
	# Threads not available nor needed on the web.
	if os_name != 'HTML5' && os_name != 'Web':
