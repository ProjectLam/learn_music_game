...
func _ready():
	spectrum_analyzer = AudioServer.get_bus_effect_instance(AudioServer.get_bus_index("Input"), 1) as AudioEffectSpectrumAnalyzerInstance
...
func _process(delta):
	...
	for note in NoteFrequency.CHROMATIC:
		...
		volume = linear_to_db(
			spectrum_analyzer.get_magnitude_for_frequency_range(min_frequency, max_frequency).x
		)
		if volume >= threshold:
			inputs.append(note)
